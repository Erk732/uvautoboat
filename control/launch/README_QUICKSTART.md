...
## 3.5) Two-machine flow (goal sender + stack host)
Use this when one machine runs the stack and another only pushes goals (e.g., "shore laptop" → "boat/sim PC").

**On both machines (before running anything):**
1. Use the same branch of this repo and build it:
    ```bash
    colcon build --merge-install
    source install/setup.bash
    ```
2. Put both hosts on the same LAN and align DDS settings:
    ```bash
    export ROS_DOMAIN_ID=17           # pick any non-default number and reuse on both
    export ROS_LOCALHOST_ONLY=0       # allow traffic off localhost
    export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp  # ensure both use the same RMW
    export ROS_IP=<this_host_ip>      # optional: pin the NIC if you have multiple interfaces
    ```
   If discovery fails, open UDP 7400-7500 (DDS discovery/data) or try a wired connection.

**Goal receiver (boat/sim PC):**
- Run `ros2 launch control all_in_one_bringup.launch.py` and leave it running. Auto-goals inside the launch are off by default (`auto_goal_enable: False`) to avoid conflicts.

**Goal sender (shore laptop):**
- After sourcing, confirm you see the goal topic:
    ```bash
    ros2 topic list | grep planning/goal
    ```
- Option A: 单次目标
    ```bash
    ros2 topic pub /planning/goal geometry_msgs/PoseStamped \
    "{header: {frame_id: 'world'}, pose: {position: {x: 120.0, y: 15.0, z: 0.0}, orientation: {w: 1.0}}}" --once
    ```
- Option B: 连续目标（本地运行，不占用船载机）
    ```bash
    ros2 run control goal_sequencer --ros-args \
      -p goals:="130,0;130,50;80,60" \
      -p pose_topic:=/wamv/pose_filtered \
      -p frame_id:=world \
      -p goal_tolerance:=1.0
    ```
  逻辑：在收到姿态后发第一个目标，距离当前目标小于 `goal_tolerance` 时自动发送下一个，直至全部完成。仅 `/planning/goal` 和 `/wamv/pose_filtered` 经过网络，避障与控制依然在接收端执行。
  若要用本地文件定义目标，创建文件例如 `/tmp/goals.txt`：
    ```text
    # x y or x,y per line
    130 0
    130 50
