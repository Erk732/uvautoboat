...
class atlantis(Node):
    def __init__(self):
        ...
        self.get_logger().info("TEST CODE AUTONOMUS NAVIGATION SYSTEM")
        self.get_logger().info("atlantis - Autonomous Navigation System")
        self.get_logger().info("+ Smart Anti-Stuck System (SASS) v2.0")
        self.get_logger().info("=" * 60)
        self.get_logger().info(f"Scanning area: {self.scan_length}m × {self.scan_width * self.lanes}m")
        self.get_logger().info(f"Stripes: {self.lanes}, Width: {self.scan_width}m")
        ...
        self.get_logger().info(f"Obstacle detection: Safely={self.min_safe_distance}m, Critical={self.critical_distance}m")
    ...
    def control_loop(self):
        ...
        if dist < self.waypoint_tolerance:
            self.get_logger().info(
                ...
                f"Waypoint reached at ({curr_x:.1f}, {curr_y:.1f})"
            )
        ...
        if self.min_obstacle_distance < self.critical_distance:
            ...
            if self.reverse_start_time is None:
                ...
                self.get_logger().warn(
                    f"CRITICAL OBS {self.min_obstacle_distance:.2f}m - Reverse! | CRITICAL OBSTACLE - Reversing!"
                )
        ...
        if self.obstacle_detected:
            ...
            self.get_logger().warn(
                f"OBSCTALE! {self.min_obstacle_distance:.1f}m - turn {direction} | "
    ...
    def log_status(self, curr_x, curr_y, target_x, target_y, dist, error):
        ...
        self.get_logger().info(
            f"WP {wp_progress} | "  # Waypoint
            f"Pos: ({curr_x:.1f}, {curr_y:.1f}) | "  # Position
            f"Target: ({target_x:.1f}, {target_y:.1f}) | "  # Target
            f"Distance: {dist:.1f}m | "  # (Distance)
            f"Error: {math.degrees(error):.1f}° | "  #Error
            f"{obs_status}"
        )
    ...
    def finish_mission(self, final_x, final_y):
        ...
        self.get_logger().info("=" * 60)
        self.get_logger().info(f"Final Position: ({final_x:.1f}m, {final_y:.1f}м)")
        self.get_logger().info(f"Total Distance: {self.total_distance:.1f}м")
        self.get_logger().info(f"Mission Time: {elapsed_min:.1f} мinutes")
        if elapsed > 0:
            avg_speed = self.total_distance / elapsed
            self.get_logger().info(f"Average Speed: {avg_speed:.2f} м/s")
        self.get_logger().info("=" * 60)
